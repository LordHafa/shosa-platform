...
app.post("/api/admin/gallery/upload", adminAuthMiddleware, upload.single("image"), (req, res) => {
...
app.post("/api/sacco/register", authMiddleware, (req, res) => {
  const { membershipType, monthlyContribution, startDate, notes } = req.body || {};
  ...
  if (existing) {
    run(
      `UPDATE sacco_memberships SET
        membershipType = :membershipType, monthlyContribution = :monthlyContribution, startDate = :startDate,
        status = :status, notes = :notes, updatedAt = :updatedAt
       WHERE alumniId = :alumniId`,
      {
        membershipType, monthlyContribution: contribution, startDate,
        status: enforcedStatus, notes: notes || null,
        updatedAt: t, alumniId: req.user.id,
      }
    );
    return res.json({ id: existing.id, alumniId: req.user.id, membershipFeePaid, status: enforcedStatus, nextRequiredStep: membershipFeePaid ? null : "pay_membership_fee" });
  }
  ...
  const info = run(
    `INSERT INTO sacco_memberships
     ...
     VALUES
      (:alumniId, :membershipType, :monthlyContribution, :startDate, :status, :notes, :createdAt, :updatedAt)`,
    {
      alumniId: req.user.id, membershipType, monthlyContribution: contribution,
      startDate, status: enforcedStatus, notes: notes || null, createdAt: t, updatedAt: t,
    }
  );
  res.json({ id: info.lastInsertRowid, alumniId: req.user.id, membershipFeePaid, status: enforcedStatus, nextRequiredStep: membershipFeePaid ? null : "pay_membership_fee" });
});
...
app.post("/api/payments/mobilemoney", authMiddleware, (req, res) => {
  ...
  const PAYMENT_TYPES = {
    ...
    sacco_savings_monthly:     { exactAmount: null,   minAmount: 50000,  label: "SACCO monthly savings deposit" },
    sacco_savings_quarterly:   { exactAmount: null,   minAmount: 200000, label: "SACCO quarterly savings deposit" },
  ...
  res.json({
    ...
    currency: "UGX",
    message: "Payment recorded. Connect to MTN/Airtel Mobile Money gateway for live processing.",
  });
});
...
app.post("/api/gallery/upload", adminAuthMiddleware, upload.single("image"), (req, res) => {
  if (!req.file) return res.status(400).json({ error: "image file is required." });
  const { category, label, year } = req.body || {};
  const url = "/uploads/" + req.file.filename;
  const info = run(
